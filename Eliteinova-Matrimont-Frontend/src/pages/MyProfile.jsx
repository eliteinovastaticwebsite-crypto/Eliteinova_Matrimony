...
const formatImageUrl = (url) => {
  ...
  // Construct final URL - try different patterns
  const finalUrl = `http://localhost:8080/api/files/images/${cleanUrl}`;
...
const tryAlternativeDocumentPaths = async (userId) => {
  ...
  for (const docType of documentTypes) {
    try {
      ...
      const testUrl = `http://localhost:8080/api/files/documents/uploads/users/${userId}/documents`;
...
const createFallbackDocuments = (profile) => {
  ...
  // Check for jathagam in various possible fields
  if (profile.jathagam || profile.jathagamFileId) {
    fallbackDocuments.push({
      id: Date.now() + 1,
      documentType: 'jathagam',
      fileName: 'Jathagam Document',
      documentUrl: profile.jathagam || `users/${profile.userId}/documents/jathagam`,
      uploadedAt: new Date().toISOString(),
      fileSize: 0,
  ...
  // Check for resume in various possible fields
  if (profile.resume || profile.resumeFileId) {
    fallbackDocuments.push({
      id: Date.now() + 2,
      documentType: 'resume',
      fileName: 'Resume Document',
      documentUrl: profile.resume || `users/${profile.userId}/documents/resume`,
      uploadedAt: new Date().toISOString(),
      fileSize: 0,
      mimeType: 'application/pdf'
    });
...
const handleDocumentUpload = async (e, documentType) => {
  ...
  try {
    ...
    // Optional: show some upload UI state if desired

    const uploadResponse = await profileService.uploadDocument(file, documentType);
    console.log(`✅ ${documentType} upload response:`, uploadResponse);

    if (uploadResponse && uploadResponse.success) {
      alert(`${documentType} uploaded successfully!`);

      // If backend returns updated documents list, use it
      if (Array.isArray(uploadResponse.documents) && uploadResponse.documents.length > 0) {
        setDocuments(uploadResponse.documents);
      } else {
        // fallback: try to re-fetch documents endpoint
        try {
          const documentsResponse = await profileService.getMyDocuments();
          if (documentsResponse && documentsResponse.success && Array.isArray(documentsResponse.documents)) {
            setDocuments(documentsResponse.documents);
      ...
      // IMPORTANT: Refresh profile so profile.jathagam / profile.resume (and fileIds) are updated
      await refreshProfile();

    } else {
      throw new Error(uploadResponse?.error || `Failed to upload ${documentType}`);
    }
  } catch (error) {
    console.error(`❌ Error uploading ${documentType}:`, error);
    alert(`Failed to upload ${documentType}: ${error.message || error}`);
  }
};
...
{activeTab === "documents" && (
  <DocumentsTab
    ...
    documents={documents} // Pass the documents state
    documentsLoading={documentsLoading} // Pass loading state
  />
)}
        </div>

        {/* Profile Stats */}
        <ProfileStats profile={profile} photosCount={photos.length} />
      </div>
    </div>
  );
};
...
const DocumentsTab = ({
  ...
  documentsLoading = false // Add loading prop
}) => {
  // Find specific documents from fetched documents
  const jathagamDoc = getDocumentByType('jathagam', documents);
  const resumeDoc = getDocumentByType('resume', documents);
  
  // Use existing profile data or fetched documents
  const displayJathagam = jathagamDoc || hasJathagam;
  const displayResume = resumeDoc || hasResume;
  ...
  return (
    <div>
      ...
      <div className="space-y-6">
        ...
        <div className="border border-gray-200 rounded-xl p-6">
          ...
          {displayJathagam ? (
            <div className="flex items-center justify-between p-4 bg-green-50 rounded-lg border border-green-200">
              <div className="flex items-center gap-4">
                <span className="text-2xl">
                  {getFileTypeIcon(jathagamDoc || profile.jathagam)}
                </span>
                <div>
                  <p className="font-medium text-gray-800">
                    {jathagamDoc?.fileName || 'Jathagam Document'}
                  </p>
                  <p className="text-sm text-gray-600">
                    {jathagamDoc?.uploadedAt ? 
                      `Uploaded ${new Date(jathagamDoc.uploadedAt).toLocaleDateString()}` : 
                      'Uploaded'}
                  </p>
                </div>
              </div>
              <div className="flex gap-2">
                <button
                  onClick={() => onPreviewFile(jathagamDoc || profile.jathagam)}
            ...
            <div className="text-center p-8 border-2 border-dashed border-gray-300 rounded-lg bg-gray-50">
              ...
              <p className="text-gray-600 mb-2">No Jathagam uploaded yet</p>
              ...
              {isEditing && (
                <label className="bg-red-600 text-white px-6 py-2 rounded-lg hover:bg-red-700 transition cursor-pointer">
                  ...
                  <input
                    ...
                    onChange={(e) => handleDocumentUpload(e, "jathagam")}
                    className="hidden"
                  />
                </label>
              )}
        ...
        <div className="border border-gray-200 rounded-xl p-6">
          ...
          {displayResume ? (
            <div className="flex items-center justify-between p-4 bg-blue-50 rounded-lg border border-blue-200">
              <div className="flex items-center gap-4">
                <span className="text-2xl">
                  {getFileTypeIcon(resumeDoc || profile.resume)}
                </span>
                <div>
                  <p className="font-medium text-gray-800">
                    {resumeDoc?.fileName || 'Resume Document'}
                  </p>
                  <p className="text-sm text-gray-600">
                    {resumeDoc?.uploadedAt ? 
                      `Uploaded ${new Date(resumeDoc.uploadedAt).toLocaleDateString()}` : 
                      'Uploaded'}
                  </p>
                </div>
              </div>
              <div className="flex gap-2">
                <button
                  onClick={() => onPreviewFile(resumeDoc || profile.resume)}
                  className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors"
                >
                  <span>👁️</span>
                  View
        ...
        {/* All Documents Section - Only show if we have documents from backend */}
        {documents.length > 0 && (
          <div className="border border-gray-200 rounded-xl p-6">
            <h3 className="text-xl font-semibold text-gray-800 mb-4 flex items-center">
              <span className="mr-2">📂</span>
...
const ProfileStats = ({ profile, photosCount }) => (
  <div className="bg-white rounded-2xl shadow-lg p-8 mt-8 border border-gray-200">
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
      ...
      />
      <StatCard
        value={profile.jathagamFileId || profile.jathagam ? "Yes" : "No"}
        label="Jathagam Uploaded"
        color="yellow"
        icon="📜"
      />
      <StatCard
        value={profile.resumeFileId || profile.resume ? "Yes" : "No"}
