...
function ReportModal({ onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    console.log("📝 Submitting suggestion form:", { reason, details });
    
    try {
      const response = await profileService.submitSuggestion(reason, details);
      ...
      if (response && response.success) {
        ...
        const errorMsg = response?.message || response?.error || "Failed to submit suggestion. Please try again.";
        console.error("❌ Suggestion submission failed:", errorMsg);
        setError(errorMsg);
        setIsSubmitting(false);
      }
    } catch (err) {
      console.error("❌ Error submitting suggestion:", err);
      const errorMsg = err.message || "Failed to submit suggestion. Please try again.";
      setError(errorMsg);
      setIsSubmitting(false);
    }
  };
      ...
      <div style={{ overflowY: "auto", backgroundColor: "#ffffff", flex: 1 }}>
        {submitted ? (
          <div className="text-center py-10 px-5">
            ...
            <p className="text-gray-500 text-xs mt-2" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
          ...
          <form onSubmit={handleSubmit} className="p-5 space-y-4">
            ...
            {error && (
              <div className="bg-red-50 border border-red-200 rounded-lg p-3">
                <p className="text-xs text-red-600" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
                  {error}
                </p>
              </div>
            )}
            ...
            <button
              type="submit"
              disabled={isSubmitting || !reason || reason.trim() === ""}
              className="w-full py-2.5 rounded-lg text-sm font-bold text-white bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2"
              style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}
            >
              {isSubmitting ? (
                <>
...
function SuccessStorySection() {
  ...
  const fetchStories = async () => {
    try {
      ...
      if (response && response.success && response.stories && Array.isArray(response.stories) && response.stories.length > 0) {
        console.log("✅ Setting stories:", response.stories.length);
        setStories(response.stories);
      } else {
        console.log("⚠️ No stories found or empty response");
        setStories([]);
      }
    } catch (err) {
      console.error("❌ Error fetching success stories:", err);
...
<button
  onClick={() => setShowShareForm(true)}
  data-success-share // Add this line
  className="flex-shrink-0 text-[11px] font-bold text-white bg-gradient-to-r from-pink-500 to-red-500 hover:from-pink-600 hover:to-red-600 px-3 py-1.5 rounded-lg transition-all shadow-sm"
  style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}
>
  Share →
...
// Then replace the return section starting from the aside element with this:
...
return (
    <>
      ...
      <aside
        ...
        style={{ minWidth: 0 }}
      >
        {/* ── Mobile: Three buttons row ── */}
        <div className="flex md:hidden gap-2 mb-2">
          {/* Wedding Service Button (Register Now) */}
          <button
            onClick={handleWeddingServiceClick}
            className="flex-1 flex flex-col items-center justify-center bg-gradient-to-br from-red-500 to-orange-500 text-white rounded-lg py-2 px-1 shadow-md"
            style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}
          >
            <span className="text-lg mb-0.5">💒</span>
            <span className="text-[10px] font-bold leading-tight">Wedding Service</span>
            <span className="text-[8px] opacity-90">Register Now</span>
          </button>

          {/* Success Stories Button (Share) */}
          <button
            onClick={() => {
              // Find and click the existing Share button
              const shareBtn = document.querySelector('[data-success-share]');
              if (shareBtn) shareBtn.click();
            }}
            className="flex-1 flex flex-col items-center justify-center bg-gradient-to-br from-pink-500 to-red-500 text-white rounded-lg py-2 px-1 shadow-md"
            style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}
          >
            <span className="text-lg mb-0.5">💑</span>
            <span className="text-[10px] font-bold leading-tight">Success Stories</span>
            <span className="text-[8px] opacity-90">Share Story</span>
          </button>

          {/* Suggestions Button (Report) */}
          <button
            onClick={() => setShowReportModal(true)}
            className="flex-1 flex flex-col items-center justify-center bg-gradient-to-br from-orange-500 to-yellow-500 text-white rounded-lg py-2 px-1 shadow-md"
            style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}
          >
            <span className="text-lg mb-0.5">📢</span>
            <span className="text-[10px] font-bold leading-tight">Suggestions</span>
            <span className="text-[8px] opacity-90">Report</span>
          </button>
        </div>

        {/* ── Mobile: Big Search Filters Button ── */}
        <div className="md:hidden mb-2">
          <button
            onClick={() => setShowMobileFilters(!showMobileFilters)}
            className="w-full py-3 px-4 rounded-lg font-bold text-white shadow-lg flex items-center justify-center gap-2"
            style={{
              fontFamily: "'Plus Jakarta Sans', sans-serif",
        ...
        {/* ── Desktop: Eliteinova Wedding Services Banner ── */}
        ...
        {/* ── Filter Panel — with data-success-share attribute for mobile share button ── */}
        <div className="hidden md:block" data-success-share-trigger>
          {/* This is just a placeholder - the actual SuccessStorySection is above */}
        </div>

        {/* ── Filter Panel — Conditionally shown on mobile, always shown on desktop ── */}
        <div
          className={`${
            showMobileFilters ? 'block' : 'hidden'
          } md:block flex flex-col rounded-lg shadow-lg relative overflow-hidden mt-2`}
          style={{
            background: currentTheme.sidebarBg,
            border: `1.5px solid ${currentTheme.sidebarBorder}`,
          }}
...
/>
                ...
                <CustomSelect label="Dosham" value={filters.dosham || ""}
                  ...
                  options={[
                    { value: "", label: "Dosham" }, { value: "Yes", label: "Yes" },
                    { value: "No", label: "No" }, { value: "Don't Know", label: "Don't Know" },
                  ]}
                />
                <CustomSelect label="Marital Status" value={filters.maritalStatus || ""}
                  onChange={(e) => handleChange("maritalStatus", e.target.value)}
              ...
              <FilterBox title="Professional Details">
                <CustomSelect label="Education" value={filters.education || ""}
                  ...
                  options={[
                    ...
                    { value: "Master's Degree", label: "Master's Degree" }, { value: "PhD", label: "PhD" },
                  ]}
                />
                {filters.education === "Other" && (
                  <Input label="Please specify education" placeholder="Please specify education"
                ...
                <CustomSelect label="Employed In" value={filters.employedIn || ""}
                  onChange={(e) => handleChange("employedIn", e.target.value)}
                  options={[
                    { value: "", label: "Employed In" }, { value: "Private Sector", label: "Private Sector" },
                    { value: "Government Sector", label: "Government Sector" }, { value: "Business", label: "Business" },
                    { value: "Self Employed", label: "Self Employed" }, { value: "Not Working", label: "Not Working" },
                  ]}
                />
                <CustomSelect label="Annual Income" value={filters.annualIncome || ""}
