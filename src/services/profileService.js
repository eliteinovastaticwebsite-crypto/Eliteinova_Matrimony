...
class profileService {
 ...
 mapFiltersToBackend(filters) {
  ...
  async getAllProfiles(page = 0, size = 12, membershipType = null) {
    try {
  ...
  async formatDocumentUrl(documentPath) {
    ...
    const { buildDocumentUrl } = await import('../config/api');
    return buildDocumentUrl(documentPath);
  }
...
async updateProfile(profileData) {
  ...
  extractPhotoPath(photoUrl) {
    ...
    // Use API utilities to get base URL
    const baseUrl = `${API_BASE_URL}/api/files/images/`;
